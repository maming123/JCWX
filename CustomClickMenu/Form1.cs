...
namespace CustomClickMenu
{
    ...
    {
        private static string s_menufilepath = "";
        ...
        {
            InitializeComponent();
            s_menufilepath=Environment.CurrentDirectory+"\\data.xml";
            ReadMenuXml();
            RefreshGrid();
            LoadAppId();
        }
            ...
            if (elements.Any())
            {
                foreach (var e in elements)
                {
                    ...
                    if (e.Element("Id") != null)
                    {
                        ...
                        if (e.Element("Media_ID") != null)
                            row.Media_ID = e.Element("Media_ID").Value;
            ...
            if (this.dataGridView1.SelectedRows.Count > 0)
            {
                ...
                if (row != null)
                {
                    ...
                    if (form.ShowDialog(this) == System.Windows.Forms.DialogResult.OK)
                    {
                        ...
                        row.Media_ID = form.Row.Media_ID;
                    }
                    RefreshGrid();
                }
            }
        ...
        private IEnumerable<ClickButton> BuildButton()
        {
            ...
            foreach (var r in Rows.Where(r => String.IsNullOrEmpty(r.RootId)))
            {
                ...
                if (r.MenuType == "View")
                {
                    button.Type = ClickButtonType.view;
                    button.Url = r.Url;
                }
                else if (r.MenuType == "Click")
                {
                    button.Type = ClickButtonType.click;
                    button.Key = r.Key;
                }
                else if (r.MenuType == "Media_ID")
                {
                    button.Type = ClickButtonType.media_id;
                    button.Media_ID = r.Media_ID;
                }
                ...
                if (subButton.Any())
                {
                    ...
                    //}).Take(5);
                    List < ClickButton > list= new List<ClickButton>();
                    foreach(DataGridRow dgr in subButton)
                    {
                        ...
                        {
                            cb.Type = ClickButtonType.media_id;
                            cb.Media_ID = dgr.Media_ID;
                        }
                       
                        list.Add(cb);
                    }
                    button.SubButton = list.Take(5);
                }
